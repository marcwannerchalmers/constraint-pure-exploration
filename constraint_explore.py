...
def project_on_feasible(allocation, A, b):
    ...
    if A is not None:
        ...
        b = np.concatenate([b, np.ones(len(allocation)),  np.zeros(len(allocation)), np.array([1]), np.array([-1])], axis=0)
    else: 
        A = np.concatenate([eye, - eye, simplex, -simplex], axis=0)
        b = np.concatenate([np.ones(len(allocation)),  np.zeros(len(allocation)), np.array([1]), np.array([-1])], axis=0)
...
def get_policy(mu, A, b):
    ...
    results = linprog(-mu, A_ub=A, b_ub=b, A_eq=None, b_eq=None, method='highs-ds')# Use simplex method
    if not results['success']:
        raise 'LP Solver failed'
...
def enumerate_all_policies(A, b):
    ...
    for base in bases:
        ...
        if np.linalg.matrix_rank(B) == A.shape[1]:
            ...
            if np.all(A.dot(policy) <= b + 1e-5) and not arreqclose_in_list(policy, policies):
        ...
        for constraint in inactive_constraints:
            ...
            for i in range(len(base)):
                ...
                if np.linalg.matrix_rank(B) == len(vertex):
                    ...
                    if np.all(A.dot(possible_neighbor) <= b + 1e-5) and not arreqclose_in_list(possible_neighbor, neighbors):
...
def get_confidence_interval(mu, pulls, f_t, upper=6, lower=-1, kl=None):
    ...
    ub = [binary_search(m, np.linspace(m, upper, 5000), threshold=f_t / n, kl=kl)[0] for m, n in zip(mu, pulls)]
    lb = [binary_search(m, np.linspace(m, lower, 5000), threshold=f_t / n, kl=kl)[0] for m, n in zip(mu, pulls)]
...
def best_response(w, mu, pi, neighbors, sigma=1, dist_type='Gaussian'):
    ...
    if dist_type == 'Gaussian':
        projections = [gaussian_projection(w, mu, pi, neighbor, sigma) for neighbor in neighbors]
    elif dist_type == 'Bernoulli':
        projections = [bernoulli_projection(w, mu, pi, neighbor, sigma) for neighbor in neighbors]
    else:
...
def solve_game(mu, vertex, neighbors, sigma=1, dist_type='Gaussian', allocation_A=None, allocation_b=None, tol=None, x0=None):
    '''
    ...
    if allocation_A is None:
        ...
        if x0 is None:
            while count < len(tol_sweep) and not done:
                ...
                x0 = x0 / x0.sum()
                tol = tol_sweep[count]
                res = minimize(game_objective, x0, constraints=constraint, bounds=bounds, tol=tol)
                done = res['success']
                count += 1
        else:
            res = minimize(game_objective, x0, constraints=constraint, bounds=bounds, tol=tol)
...
class Explorer:
    ...
    '''
    def __init__(self, n_arms, A, b, delta,
                  ini_phase=1, sigma=1, restricted_exploration=False, dist_type='Gaussian', seed=None, d_tracking=False):
        '''
        ...
        if dist_type == 'Gaussian':
            ...
            self.kl = lambda x, y: x * np.log(x / y) + (1 - x) * np.log((1 - x) / (1 - y))
            self.lower = 0 + 1e-4
            self.upper = 1 - 1e-4
            self.ini_phase = 10 # Take longer initial phase for Bernoulli to avoid all 0  or all 1
        else:
            raise NotImplementedError
    ...
    def stopping_criterion(self, vertex):
        ...
        game_value, _ = best_response(w=self.empirical_allocation(),
                                        mu=self.means,
                                        pi=vertex, 
...
class TnS(Explorer):
    ...
    '''
    def __init__(self, n_arms, A, b, delta,
                  ini_phase=1, sigma=1, restricted_exploration=False, dist_type='Gaussian', seed=None, d_tracking=True):
        '''
        Initialize the explorer
        :param n_arms: number of arms
        :param A: matrix constraints
        ...
        '''
        super().__init__(n_arms, A, b, delta, ini_phase, sigma, restricted_exploration, dist_type, seed, d_tracking)
    ...
    def act(self):
        ...
        allocation, game_value = solve_game(mu=self.means,
        ...
        misc = {'game_value': game_value, 'allocation': allocation, 'optimal_policy': optimal_policy}
...
class CGE(Explorer):
    ...
    def __init__(self, n_arms,
        ...
        if use_adahedge:
            ...
            if restricted_exploration:
                ...
                self.ada = AdaGrad(A=allocation_A, b=allocation_b, loss_rescale=1) #AdaHedge(d=n_arms, loss_rescale=0.01)
        else:
    ...
    def act(self):
        ...
        br_value, br_instance = best_response(w=allocation, mu=self.means,
                                               pi=optimal_policy,
                                                 neighbors=neighbors, 
                                                 sigma=self.sigma,
                                                   dist_type=self.dist_type)
        ...
        ft = np.log(self.t)
        # Optimism
        lb, ub = get_confidence_interval(self.means, self.n_pulls, ft, kl=self.kl, lower=self.lower, upper=self.upper)
        loss = [np.max([ft / self.n_pulls[a], 
        ...
        misc = {'br_value': br_value, 'br_instance': br_instance, 'allocation': allocation, 'optimal_policy': optimal_policy}
...
class UniformExplorer(Explorer):
    ...
    def __init__(self, n_arms, A, b, delta, ini_phase=1, sigma=1, restricted_exploration=False, dist_type='Gaussian', seed=None,
                 allocation=None):
        super().__init__(n_arms, A, b, delta, ini_phase, sigma, restricted_exploration, dist_type, seed)
...
class AdaHedge:
    ...
    def update(self, loss):
        ...
        if self.t == 1:
            ...
            delta = self.gamma * np.log(np.sum(self.w * np.exp(- loss / self.gamma))) + total_loss
...
if __name__ == '__main__':
    ...
    explorer = TnS(len(mu), A=A, b=b, delta=delta, restricted_exploration=True, dist_type='Gaussian')
